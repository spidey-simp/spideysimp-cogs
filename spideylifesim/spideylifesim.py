...
class SpideyLifeSim(Cog):
    ...
    async def slsstore_storeview(self, ctx: commands.Context):
        ...
        store_items_text = "\n".join(
            [f"**Slot {i + 1}:** {item} {currency} for {store_slots[i]}" for i, item in enumerate(store_items)]
        ) + f"\n{cooldown_timer}"
    ...
    async def slsstore_purchase(self, ctx: commands.Context, storeslot: int = 0, itemcount: int = 1):
        ...
        await ctx.send(f"You have successfully purchased {itemcount} {item_for_purchase}{item_plural}. "
                   f"Remaining balance: {user_balance} {currency}.")
    ...
    async def slsstore_sellitem(self, ctx: commands.Context, count: int = 1, *, itemtosell: str = None):
        ...
        await ctx.send(f"You sold {count} {itemtosell}{item_plural}. New balance: {user_balance} {currency}.")
    ...
    async def slsskills_skillprogress(self, ctx: commands.Context):
        ...
        skillresult = "```Here are your skill levels:\n" + "\n".join(
            f"- {key.ljust(20)}Level {value[0]} {generate_progress_bar(value[1])}"
    ...
    async def slscareers_gotowork(self, ctx: commands.Context):
        ...
        if newjobperc >= 100:
            if all(promotionlevel <= skill_value for skill_value in skill_values):
                ...
                await ctx.send(
                    ...
                    f"New salary: {promotionsalary} {currency}, with a bonus of {promotionbonus} {currency}."
                )
            else:
                newjobperc = 99
        ...
        await ctx.send(f"Worked at {careerfield}, earning {salary} {currency}.")
